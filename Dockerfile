...
RUN \
    ...
    $HOME/miniconda/bin/conda config --system --add channels conda-forge && \
    $HOME/miniconda/bin/conda config --system --set channel_priority strict && \
    $HOME/miniconda/bin/conda create -y -n pyfmi3 \
        python=3.10 \
        pyfmi=2.11 \
        pandas=1.5.3 \
        matplotlib=3.7.1 \
        ...
        flask-restful=0.3.9 \
        werkzeug=2.2.3 && \
    $HOME/miniconda/envs/pyfmi3/bin/pip install torch && \
